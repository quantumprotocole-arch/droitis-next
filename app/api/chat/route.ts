...
async function createEmbedding(input: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `OpenAI embeddings error: ${res.status} ${res.statusText} — ${JSON.stringify(err)}`
    )
  }
...
async function createChatCompletion(
  ...
  if (!res.ok) {
    const err = await safeJson(res)
    throw new Error(
      `OpenAI chat error: ${res.status} ${res.statusText} — ${JSON.stringify(err)}`
    )
  }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const sources: Source[] = topContext.map((h, idx) => ({
      id: h.id ?? `S${idx + 1}`,
